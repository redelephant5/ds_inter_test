...
func getMetadata(name string, versionId int)(meta Metadata, e error) {
	urlStr := fmt.Sprintf("http://%s/metadata/objects/%s_%d/_source",
...
func PutMetadata(name string, version int, size int64, hash string) error {
	doc := fmt.Sprintf(`{"name":"%s","version":%d,"size":%d,"hash":"%s"}`, name, version,
		size, hash)
	client := http.Client{}
	urlStr := fmt.Sprintf("http://%s/metadata/objects/%s_%d?op_type=create",
